...
export default function Sidebar() {
  ...
  const getAllThreads = async () => {
    try {
      const response = await fetch("https://chatgpt-clone-cxt6.onrender.com/api/thread");
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(
        `https://chatgpt-clone-cxt6.onrender.com/api/thread/${newThreadId}`
      );
      const res = await response.json();
      setPreviousChat(res);
      setNewChat(false);
  ...
  const handleDeleteThread = async (threadId, e) => {
    e.stopPropagation();
    try {
      const response = await fetch(`https://chatgpt-clone-cxt6.onrender.com/api/thread/${threadId}`, {
        method: "DELETE",
      });
